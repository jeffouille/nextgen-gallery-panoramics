	...
	function tinyselect() {
        ...
        switch ( $this->type ) {
            case 'image' :
                ...
                if ( empty($this->term) )
                    $list = $nggdb->find_all_galleries('gid', 'DESC', true, $this->limit );
                else
                ...
                if( is_array($list) ) {
                    foreach($list as $gallery) {
                        ...
                        $obj->value = $name;
                        $obj->counter = $gallery->counter;
                        
                        $photos_array = array();
                        // get picture values
                        $picturelist = $nggdb->get_gallery($gallery->gid, $ngg->options['galSort'], $ngg->options['galSortDir'], true, $this->limit,0, true );
                        if( is_array($picturelist) ) {
                            ...
                            foreach($picturelist as $image) {
                                ...
                                $name = stripslashes( htmlspecialchars_decode($name, ENT_QUOTES));
                                $pobj->label = $image->pid . ' - ' . $name;
                                $pobj->value = $name;
                                $photos_array[] = $pobj;
                            }
                        }
                        
                        $obj->pictures = $photos_array;
